...
protected:
    Optimizer<float>::LearningRateSchedule::StepDecaySchedule lr_schedule;
    Optimizer<float>::Adam optimizer;

    AdamTest()
        : OptimizerTest<float>(),
          lr_schedule(0.01f, 0.1f, 100),
          optimizer(0.01f, lr_schedule) {}
};
...
TEST_F(AdamTest, HandleLargeGradients) {
    SetUpData(Tensor<float>({2, 2}, std::vector<float>{0.1, 0.2, 0.3, 0.4}),
              ...
              Tensor<float>({2, 2}, std::vector<float>{0.090227, 0.190227, 0.290227, 0.390227}));
